...
describe('cleanServerName', () => {
  ...
  test('should extract hostname from text with URL prefix', () => {
    expect(
      cleanServerName('URL for registration services: https://example.com whois.nic.com')
    ).toBe('whois.nic.com');
  });
...
describe('cleanRdapUrl', () => {
  ...
  test('should handle URLs with query parameters', () => {
    expect(cleanRdapUrl('https://rdap.example.com?query=test')).toBe(
      'https://rdap.example.com?query=test'
    );
  });

  test('should handle URLs with fragments', () => {
    expect(cleanRdapUrl('https://rdap.example.com#section')).toBe(
      'https://rdap.example.com#section'
    );
  });
  ...
  test('should handle complex RDAP URLs', () => {
    expect(cleanRdapUrl('https://rdap.nic.xn--mgba3a3ejt/')).toBe(
      'https://rdap.nic.xn--mgba3a3ejt/'
    );
    expect(cleanRdapUrl('https://tld-rdap.verisign.com/com/v1/')).toBe(
      'https://tld-rdap.verisign.com/com/v1/'
    );
  });
});
